...
yunlin_c<-c("斗南鎮","虎尾鎮","西螺鎮","北港鎮","莿桐鄉","林內鄉","崙背鄉","褒忠鄉","台西鄉","口湖鄉")
...
yunlin_3<- read.csv(str_c("109年公告土地現值暨公告地價-東勢鄉.csv"),sep =",")
yunlin_4<- read.csv(str_c("109年公告土地現值暨公告地價-麥寮鄉.csv"),sep =",")

#欄位改名
colnames(yunlin_1)<-col
colnames(yunlin_2)<-col
colnames(yunlin_3)<-col
colnames(yunlin_4)<-col
#value、land改num
yunlin_3$land<-as.numeric(yunlin_3$land)
yunlin_4$land<-as.numeric(yunlin_4$land)
...
#輸出combine
save(combine, file = "220407_RA_花蓮縣到澎湖縣combine.RData")
