...
#####【town】
#檔案載入
town_all<-read_excel(str_c("220113_RA_人房地_town_all_revise.xlsx"))

#轉數字
town_all$A<-as.numeric(town_all$A)
town_all$B<-as.numeric(town_all$B)
town_all$C1<-as.numeric(town_all$C1)
town_all$C2<-as.numeric(town_all$C2)
town_all$D1<-as.numeric(town_all$D1)
town_all$D2<-as.numeric(town_all$D2)
town_all$E<-as.numeric(town_all$E)
town_all$FF<-as.numeric(town_all$FF)
town_all$NU<-as.numeric(town_all$NU)

#category名稱，要數字轉特定中文   
adj_edu_town<-filter(town_all,type=="教育")%>%
  mutate(adj_category=if_else(category=="1","國小與國小以下",
                              if_else(category=="2","大學以上",
                                      if_else(category=="3","大學",
                                              if_else(category=="4","高中",
...
adj_marriage_town<-filter(town_all,type=="婚姻")%>%
  mutate(adj_category=if_else(category=="1","未婚",
                              if_else(category=="2","有偶",
                                      if_else(category=="3","離婚","喪偶"))),
         order=if_else(category=="1",1,
...
adj_house_town<-filter(town_all,type=="家戶組成")%>%
  mutate(adj_category=if_else(category=="1","單人",
                              if_else(category=="2","夫婦",
                                      if_else(category=="3","三代",
                                              if_else(category=="4","祖孫",
...
adj_age_town<-filter(town_all,type=="年齡組別")%>%
  mutate(adj_category=if_else(category=="1","0  -19 ",
                              if_else(category=="2","20 - 24 ",
                                      if_else(category=="3","25 - 29 ",
                                              if_else(category=="4","30 - 34 ",
                                                                                                       ...
                                                                                                       if_else(category=="12",12,
                                                                                                               if_else(category=="13",13,
                                                                                                                       if_else(category=="14",14,
                                                                                                                               if_else(category=="15",15,16))))))))))))))))
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#合併已改名的category回到town_all 
town_all_adj<-bind_rows(adj_edu_town,adj_marriage_town,adj_house_town,adj_age_town)
...
for (j in 1:4) {
  for (i in 1:368) {
    list[[j]][[i]]<-filter(town_all_adj,code==towncode[i],type==sheet[j],year==2020)%>%
      mutate(a= A,
             aa= percent(A/(A+C1+C2+D1+D2+E+FF)),
             b= C1+D1,
             bb= percent((C1+D1)/(A+C1+C2+D1+D2+E+FF)),
      ...
      arrange(type2,order)%>%
      subset(select = c(-category,-A, -B, -C1,-C2,-D1,-D2,-E,-FF,-NU,
                        -year,-town,-code,-type,-order))%>%
      rbind(Newrow=c(" "),c(" "),c(" "),c(" "),c(" "),c(" "))%>%
      rbind(Newrow=c(str_c("109年度擁屋型態 按",sheet[j],"分類[",townname[i+1],"]"),
                     " "," "," "," "," "," "," "," ",
                     " "," "," "," "," "," "," "," "," "," "," "," "," "," "))%>%
      rbind(Newrow=c(sheet[j],"擁屋者"," "," "," "," "," "," "," "," "," ",
                     "無屋者"," ","其他"," "," "," "," "," "," "," "))%>%
...
#整合&輸出
town_109_table<-bind_rows(list)
write_xlsx(town_109_table,"220115_RA_人房地_town_table_109.xlsx")
...
for (j in 1:4) {
  for (i in 1:368) {
    list[[j]][[i]]<-filter(town_all_adj,code==towncode[i],type==sheet[j],year==2019)%>%
      mutate(a= A,
             aa= percent(A/(A+C1+C2+D1+D2+E+FF)),
             b= C1+D1,
             bb= percent((C1+D1)/(A+C1+C2+D1+D2+E+FF)),
      ...
      subset(select = c(-category,-A, -B, -C1,-C2,-D1,-D2,-E,-FF,-NU,
                        -year,-town,-code,-type,-order))%>%
      rbind(Newrow=c(" "),c(" "),c(" "),c(" "),c(" "),c(" "))%>%
      rbind(Newrow=c(str_c("108年度擁屋型態 按",sheet[j],"分類[",townname[i+1],"]"),
                     " "," "," "," "," "," "," "," ",
                     " "," "," "," "," "," "," "," "," "," "," "," "," "," "))%>%
      rbind(Newrow=c(sheet[j],"擁屋者"," "," "," "," "," "," "," "," "," ",
                     "無屋者"," ","其他"," "," "," "," "," "," "," "))%>%
...
#整合&輸出
town_108_table<-bind_rows(list)
write_xlsx(town_108_table,"220115_RA_人房地_town_table_108.xlsx")
...
for (j in 1:4) {
  for (i in 1:368) {
    list[[j]][[i]]<-filter(town_all_adj,code==towncode[i],type==sheet[j],year==2018)%>%
      mutate(a= A,
             aa= percent(A/(A+C1+C2+D1+D2+E+FF)),
             b= C1+D1,
             bb= percent((C1+D1)/(A+C1+C2+D1+D2+E+FF)),
      ...
      subset(select = c(-category,-A, -B, -C1,-C2,-D1,-D2,-E,-FF,-NU,
                        -year,-town,-code,-type,-order))%>%
      rbind(Newrow=c(" "),c(" "),c(" "),c(" "),c(" "),c(" "))%>%
      rbind(Newrow=c(str_c("107年度擁屋型態 按",sheet[j],"分類[",townname[i+1],"]"),
                     " "," "," "," "," "," "," "," ",
                     " "," "," "," "," "," "," "," "," "," "," "," "," "," "))%>%
      rbind(Newrow=c(sheet[j],"擁屋者"," "," "," "," "," "," "," "," "," ",
                     "無屋者"," ","其他"," "," "," "," "," "," "," "))%>%
...
#整合&輸出
town_107_table<-bind_rows(list)
write_xlsx(town_107_table,"220115_RA_人房地_town_table_107.xlsx")